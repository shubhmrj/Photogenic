...
(function() {
    ...
    function applyFilters() {
        ...
        items.forEach(item => {
            ...
            if (activeFilters.dateRange && (activeFilters.dateRange.from || activeFilters.dateRange.to)) {
                const itemDate = item.querySelector('.collection-date')?.textContent;
                if (itemDate) {
    ...
    function deleteSelected() {
        ...
        const actions = [
            { text: 'Undo', type: 'primary' }
        ];
        
        showToast('warning', 'Items Deleted', `${selectedItems.size} items moved to trash`, actions);
        clearSelection();
    }
    
    function deleteItem(item) {
        const name = item.querySelector('.item-title')?.textContent;
        showToast('warning', 'Item Deleted', `${name} moved to trash`);
    }
