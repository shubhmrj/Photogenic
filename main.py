from flask import Flask, render_template, request, jsonify, send_from_directory, redirect, url_for, session, flash, send_file
import os
import shutil
import time
import json
...
class SharedFile(db.Model):
    ...
    shared_with = db.relationship('User', foreign_keys=[shared_with_id], backref=db.backref('shared_with_me', lazy='dynamic'))
...
def generate_story():
    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                ChatCompletionSystemMessageParam(role="system", content="You are a creative storyteller that writes vivid, emotional, and engaging short stories."),
                ChatCompletionUserMessageParam(role="user", content=f"Write a vivid, creative, emotional, and detailed short story based on this input. Keep it under 500 words.\n\nInput: {story_prompt}\n\nStory:")
            ],
            max_tokens=800,
            temperature=0.7
        )
    ...
    try:
        ...
        try:
            ...
            chat_completion = clients.chat.completions.create(
                ...
                messages=[
                    ChatCompletionSystemMessageParam(role="system", content="You are a creative storyteller that writes vivid, emotional, and engaging short stories."),
                    ChatCompletionUserMessageParam(role="user", content=f"Write a vivid, creative, emotional, and detailed short story based on this input. Keep it under 300 words.\n\nInput: {prompt}\n\nStory:")
                ],
                max_tokens=600,
        ...
        if 'profile_image' in request.files:
            ...
            if profile_image.filename:
                ...
                if '.' in profile_image.filename and profile_image.filename.rsplit('.', 1)[1].lower() in allowed_extensions:
                    ...
                    filename = secure_filename(f"{current_user.username}_{int(time.time())}.{profile_image.filename.rsplit('.', 1)[1].lower()}")
